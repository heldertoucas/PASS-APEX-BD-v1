CREATE OR REPLACE PACKAGE BODY SEGURANCA_PKG AS
    ...
    BEGIN
        -- Bypass 1: Always allow access for Workspace Administrators.
        IF apex_acl.is_workspace_administrator(p_user_name => v('APP_USER')) THEN
            RETURN TRUE;
        END IF;

        -- Bypass 2: Allow access for developers running from the App Builder.
        IF v('APP_BUILDER_SESSION_ID') IS NOT NULL THEN
            RETURN TRUE;
        END IF;

        -- If not an admin or developer, proceed with the standard role check.
        ...
        EXCEPTION
            WHEN NO_DATA_FOUND THEN
                -- User is authenticated but not in our custom user table.
                RETURN FALSE;
        END;
    END is_role;
